...
export default function SpecialAttentionPanel() {
  // Special Attention rows
  ...
  const handleReset = () => {
    if (window.confirm("Reset all fields?")) {
      setRows([{ id: Date.now(), dateOfService: "", pageNo: "", comments: "" }]);
      setApsStartDate("");
      setApsEndDate("");
    }
  };
  ...
  const handleSave = () => {
    const allData = { specialAttention: rows, apsDataRange: { apsStartDate, apsEndDate } };
    alert("Special Attention & APS Data Saved!");
    console.log(allData);
  };
    ...
    <div className="w-full bg-white min-h-screen pb-2 overflow-x-hidden">
      <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-3 sm:px-4 py-3 shadow-sm">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 max-w-[1400px] mx-auto">
          <h1 className="text-lg sm:text-xl font-semibold text-black">Special Attention & APS Range</h1>
      ...
      <div className="px-2 sm:px-2 md:px-2 py-3 sm:py-2 space-y-4 max-w-[1400px] mx-auto">
        {/* Special Attention Section */}
        <div className="border border-blue-300 rounded-xl p-2 sm:p-2 bg-white shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2"><AlertCircle size={16} className="text-blue-600" /><span className="text-xs font-semibold text-black">Special Attention Entries</span></div>
        ...
        {/* APS Data Range Section */}
        <div className="border border-blue-300 rounded-xl p-3 sm:p-4 bg-white shadow-sm">
          <h2 className="text-xs sm:text-sm font-bold uppercase flex items-center gap-2 mb-3 text-black"><Calendar size={16} className="text-blue-600" /> APS DATA RANGE</h2>
