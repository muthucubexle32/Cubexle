...
import { RotateCcw, Trash2, Plus, Save, X, FileText } from "lucide-react";

// Tooltip Component (same as in SocialHistoryPanel)
...
export default function PmhPanel() {
    ...
    const handleSave = () => { alert("PMH Data Saved!"); console.log({ pmh: rows }); };
        ...
        <div className="w-full bg-white min-h-screen pb-2 overflow-x-hidden">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-3 sm:px-4 py-3 shadow-sm">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 max-w-[1400px] mx-auto">
                    <h1 className="text-lg sm:text-xl font-semibold text-black">PMH (Past Medical History)</h1>
            ...
            <div className="px-2 sm:px-2 md:px-2 py-3 sm:py-2 max-w-[1400px] mx-auto">
                <div className="border border-blue-300 rounded-xl p-2 sm:p-2 bg-white shadow-sm">
                    <div className="flex justify-between items-center mb-3">
                        <div className="flex items-center gap-2"><FileText size={16} className="text-blue-600" /><span className="text-xs font-semibold text-black">PMH Entries</span></div>
                    ...
                    <div className="space-y-3 max-h-[600px] overflow-y-auto custom-scrollbar">
                        {rows.map((row) => (
                            <div key={row.id} className="relative bg-white rounded p-2 border border-blue-100">
                                {rows.length > 1 && (
                                    <Tooltip text="Remove entry" position="right"><button onClick={() => removeRow(row.id)} className="absolute -top-4  bg-red-400 hover:bg-red-500 text-white p-1 rounded-full shadow-md"><X size={10} /></button></Tooltip>
                                )}
                                ...
                                <div><label className={labelClass}>Medical Condition / Diagnosis</label><AutoResizeTextarea value={row.comments} onChange={(e) => updateRow(row.id, "comments", e.target.value)} placeholder="Enter medical history..." className="w-full min-h-[50px] p-2 text-xs sm:text-sm text-black bg-[#CFE8F2] rounded-lg" /></div>
                            </div>
                        ))}
                    </div>
                </div>
